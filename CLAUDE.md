...
- CI locally: `just ci` (dogfoods giton via `giton.json`)

# Architecture

- Go source in `cmd/giton/`, packaged via `buildGoModule` + `makeWrapper` for runtime PATH
...
- Multi-step mode self-invokes the giton binary for each step with `--sha` and `--workdir` flags. These are internal — `--workdir` skips archive extraction since the parent already extracted once per system.
